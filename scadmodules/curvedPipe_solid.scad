...
module pipeCurve(points,point,radii, od,id,isLastSegment=false) {
	...
	// start
	translate(start) orientate(dir1) translate([0,0,preInset]) cylinder(h=l1-preInset-inset,r=od/2, center=false);

	//end
	translate(mid) orientate(dir2) translate([0,0,inset]) cylinder(h=l2-postInset-inset,r=od/2, center=false);
	...
	translate([-r,0,0]) torusSlice(r1=r, r2=od/2, r3=0, start_angle=0, end_angle=ang);
}
...
module curvedPipe(points, segments, radii, od, id) {
	union() {
        ...
        else
        {
            ...
            l1 = mod(dir1);
            translate(points[0]) orientate(dir1) cylinder(h=l1,r=od/2, center=false);
...
if (true) {
    ...
    cylinder(h=250, r=20/2);
    //tube(h=250,or=20/2, ir=19/2, center=false);
